...
export default function AbsensiPage() {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      (pos) => setKoordinat({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
      () => alert('Aktifkan GPS.')
    )
  }, [])
  ...
  const ambilFoto = () => {
    ...
    canvas.toBlob((blob) => { if (blob) setFotoBlob(blob) }, 'image/jpeg', 0.7)
    video.srcObject instanceof MediaStream && video.srcObject.getTracks().forEach(t => t.stop())
    ...
    return (
      <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-6">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 flex flex-col items-center max-w-sm w-full shadow-2xl">
          ...
          <h2 className="text-xl font-semibold text-white mb-1">Absensi Berhasil!</h2>
          <p className="text-gray-400 text-sm mb-1">Nama: <span className="text-white font-medium">{nama}</span></p>
          <p className="text-gray-500 text-xs mb-6">
            {koordinat?.lat.toFixed(5)}, {koordinat?.lng.toFixed(5)}
          </p>
          <button
            onClick={() => { setNama(''); setFoto(null); setFotoBlob(null); setStatus('') }}
            className="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 rounded-xl font-medium transition-colors"
          >
            Absensi Lagi
          </button>
          <a href="/rekap" className="mt-3 text-sm text-gray-500 hover:text-gray-300 transition-colors">
            Lihat Rekap →
          </a>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-6">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl w-full max-w-sm p-8">
        ...
        <div className="mb-7 text-center">
          ...
          <h1 className="text-xl font-semibold text-white">Absensi Karyawan</h1>
        ...
        {/* Nama */}
        <div className="mb-5">
          <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase tracking-wider">Nama Karyawan</label>
          <input
            type="text"
            value={nama}
        ...
        {/* Kamera */}
        <div className="mb-5">
          <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase tracking-wider">Foto</label>
          <div className="w-full aspect-square bg-gray-800 border border-gray-700 rounded-xl overflow-hidden relative flex items-center justify-center">
            ...
            />
            {foto && <img src={foto} alt="preview" className="w-full h-full object-cover" />}
          </div>
        ...
        {/* Lokasi */}
        <div className="mb-7">
          <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase tracking-wider">Lokasi</label>
        ...
        {status === 'error' && (
          <p className="text-red-400 text-xs text-center mt-3">Gagal menyimpan. Coba lagi.</p>
        )}

        <div className="mt-4 text-center">
          <a href="/rekap" className="text-xs text-gray-600 hover:text-gray-400 transition-colors">
            Lihat rekap absensi →
          </a>
        </div>
      </div>
